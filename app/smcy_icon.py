...
_DECODE_BUDGET_PER_WIN = 6      # 每流每 ~16ms 窗口最多解码帧数（防多台风随机 seek 卡顿）
...
class _VideoStream:
    ...
    def __init__(self, path: str, loop: bool = False) -> None:
        ...
        self._sorted_keys: list = []   # 法10: 缓存键有序列表(最近邻二分)
        self._orig_w: int = 0
        self._orig_h: int = 0
    ...
    def set_scale_size(self, size: Tuple[int, int]) -> None:
        if self._scale_size != size:
            ...
            self._cache.clear()
            self._sorted_keys.clear()   # 与 get_frame 尺寸分支一致,防最近邻命中陈旧键
    ...
    def get_frame(self, idx: int, target_size: Optional[Tuple[int, int]] = None) -> Optional[pygame.Surface]:
        ...
        if self._decode_n >= _DECODE_BUDGET_PER_WIN and self._cache:
            # 法10: 有序键二分找最近邻 O(log C)
            p = bisect.bisect_left(self._sorted_keys, idx)
            if self._loop:
                cand = set()
                if p < len(self._sorted_keys):
                    cand.add(self._sorted_keys[p])
                if p > 0:
                    cand.add(self._sorted_keys[p - 1])
                cand.add(self._sorted_keys[0])
                cand.add(self._sorted_keys[-1])
                nearest = min(cand,
                              key=lambda k: min(abs(k - idx),
                                                self._frame_count - abs(k - idx)))
            else:
                if p >= len(self._sorted_keys):
                    nearest = self._sorted_keys[-1]
                elif p == 0:
                    nearest = self._sorted_keys[0]
                else:
                    a, b = self._sorted_keys[p - 1], self._sorted_keys[p]
                    nearest = a if idx - a <= b - idx else b
        ...
        bisect.insort(self._sorted_keys, idx)
        cap = self._cache_cap()
        while len(self._cache) > cap:
            old = next(iter(self._cache))
            self._cache.popitem(last=False)
            pos = bisect.bisect_left(self._sorted_keys, old)
            if pos < len(self._sorted_keys) and self._sorted_keys[pos] == old:
                self._sorted_keys.pop(pos)
        return surf

    def preload_window(self, start_idx: int, count: int = 30,
                       target_size: Optional[Tuple[int, int]] = None) -> None:
        """预加载 [start_idx, start_idx+count) 帧到缓存，避免后续 seek 卡顿。
        K21: 传 target_size 与绘制尺寸一致,防止 get_frame 因尺寸变化清空整表。"""
        for i in range(start_idx, min(start_idx + count, self._frame_count)):
            self.get_frame(i, target_size)
...
class SMCYIconManager:
    ...
    def has_frame_window(self, category: str, hemisphere: str,
        ...
        if stream is None:
            return False
        cache = stream._cache
        for i in range(frame_idx, min(frame_idx + count, stream._frame_count)):
            if i not in cache:
                return False
        return True
        ...
        if self._icon_size_cache > 0:
            if _is_ex_category(category) and self._ex_icon_size_cache > 0:
                ...
                if ow <= 0 or oh <= 0:
                    ...
                    scale = self._ex_icon_size_cache / max(ow, oh)
                    stream.set_scale_size((max(1, int(ow * scale)), max(1, int(oh * scale))))
            else:
                stream.set_scale_size((self._icon_size_cache, self._icon_size_cache))
    ...
    for cat in sorted(categories):
        ...
        for hemi in (HEMISPHERE_NORTH, HEMISPHERE_SOUTH):
            ...
            if key in mgr._streams:
                available -= 1
                if target > 0:
                    stream = mgr._streams[key]
                    ow, oh = stream.orig_size
                    if ow <= 0 or oh <= 0:
                        stream.get_frame(0, (target, target))
                    else:
