...
class TyList(DraggableDialog):
    ...
    def _get_row_hash(self, ty) -> str:
        ...
        info = f"{cat} {mw}kt  ACE:{ty.tace:.4f}  起始:{st}"
        return f"{disp}|{info}"
    ...
    def _jump_to_typhoon_start(self, idx):
        ...
        for t in self.sim.tys:
            t.rst()
            if not t.pts:
                continue
            try:
                st_dt = datetime.strptime(t.pts[0]['t'][:10], "%Y%m%d%H")
                et_dt = datetime.strptime(t.pts[-1]['t'][:10], "%Y%m%d%H")
            except Exception:
                continue
            if target < st_dt:
                t.ss = t.act = t.sf = False
            elif target > et_dt:
                t.sf = True
                t.act = t.ss = False
            else:
