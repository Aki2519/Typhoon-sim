...
class PointList(DraggableDialog):
    ...
    def _get_row_data(self, pt: dict):
        """返回 (hash_str, {header: surface})。"""
        cols = [
            pt['t'], f"{pt['la']:.1f}", f"{pt['lo']:.1f}",
    ...
    def _apply_point_change(self, idx, vals, is_new, before=True, name=""):
        ...
        st = vals['type'] if vals['type'] else self._infer_type(w)
        ...
        if not is_new:
            ...
            pt = {'t': t, 'la': la, 'lo': lo, 'w': w, 'p': p, 'st': st, 'name': name,
    ...
    def _save(self, ty):
        if not ty.filepath:
            return
        self.sim.ensure_simple_bdeck_copy(ty)
        lines = []
        for pt in ty.pts:
    ...
    def draw(self, surface):
        ...
        for i in range(start, end):
            ...
            if h is None or h != (dh := self._get_row_data(pt)[0]):
