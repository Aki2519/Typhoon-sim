...
from .chart_helpers import (compute_path_km_in_window, _haversine_chain,
                            build_basin_order, draw_vscrollbar)
...
def _fit_col_text(text: str, max_w: int, color=None) -> pygame.Surface:
    ...
    color = color or TXT
    if max_w <= 0:
        return rt(f_s, text, color)
    full = rt(f_s, text, color)
    if full.get_width() <= max_w:
        return full
    lo, hi = 1, len(text)
    while lo < hi:
        mid = (lo + hi + 1) // 2
        if rt(f_s, text[:mid] + "…", color).get_width() <= max_w:
            lo = mid
        else:
            hi = mid - 1
    return rt(f_s, text[:lo] + "…", color)
...
class PathLengthViewer(DraggableDialog):
    ...
    def activate(self):
        ...
        for ty in self.sim.tys:
            if not ty.pts:
                continue
            # F1/N4: 按 ACE 年窗口切分(南半球 7/1-7/1)
            ts_pts = [p for p in ty.pts
                      if _ts_eligible(p)
                      and engine.point_in_limit(p['la'], p['lo'])]
            if self.sim.hemisphere == 'south':
                win = (datetime(year, 7, 1, 0), datetime(year + 1, 7, 1, 0))
                path = compute_path_km_in_window(ts_pts, year, win)
            else:
                path = compute_path_km_in_window(ts_pts, year)
            if path > 0:
    ...
    def _render(self):
        ...
        for i in range(self._scroll_offset,
            ...
            for j, (val, (_, cw)) in enumerate(zip(vals, cols)):
                ...
                vs = rt(f_s, val, clr)
                # 名称列超宽时截断,避免与下一列数值重叠
                if j == 1 and vs.get_width() > cw:
                    vs = _fit_col_text(val, cw, clr)
                surf.blit(vs, (cx, row_y))
                cx += cw + 5
