...
📖 **[使用说明.md](使用说明.md)** —— 面向第一次使用的用户：运行方式、主界面速览、
四种模式、全部快捷键、编辑台风数据、统计图表含义、脚本格式、架空模拟器、常见问题。

> 提示：主程序界面全部是中文；若看到成片英文标题/单位（SLP、hPa、kt、dam…），
> 那是 `simulator/` 子项目导出的科研图，不是主界面（见使用说明第 8 节）。
...
| 功能栏「镜头跟踪」 | 镜头跟随（风季：跟随活跃台风；正常：跟随当前台风，播完后自动完整显示该路径；仅按钮入口） |
| 右键拖拽 | 平移地图 |
| 滚轮 | 缩放地图 |
