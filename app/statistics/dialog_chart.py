...
class ACEChartDialog(ChartGridMixin, DraggableDialog):
    ...
    def deactivate(self):
        super().deactivate()
        self.dragging = False
        if self._jump_field is not None:
            self._jump_field.deactivate()   # 关闭时停用跳转字段(IME 残留防护)
            self._jump_field = None
        self._invalidate_caches()
        self._stats_data = None
    ...
    def _rebuild(self):
        ...
        # K15: 切年/重建后台风柱状图页码越界校正
        from .typhoon_ace_chart import BAR_MAX_PER_PAGE
        tp = max(1, (len(self._chart_data.typhoon_sort_data)
                     + BAR_MAX_PER_PAGE - 1) // BAR_MAX_PER_PAGE)
        if getattr(self, '_bar_page', 0) >= tp:
            self._bar_page = max(0, tp - 1)
    ...
    def draw(self, surface: pygame.Surface):
        ...
        # N5: 运行时间模式下播放推进时刷新(节流,避免每帧全量重建)
        if self.cumulative_to_current:
            now_time = (getattr(self.sim, 'sy', 0), getattr(self.sim, 'st', ''))
            if now_time != getattr(self, '_seen_sim_time', None):
                self._seen_sim_time = now_time
                if pygame.time.get_ticks() - getattr(self, '_last_rebuild_ms', 0) > 200:
                    self.needs_update = True
        if self.needs_update:
            self._rebuild()
            self.needs_update = False
            self._last_rebuild_ms = pygame.time.get_ticks()
        self._compute_layout()
        if dark:
            self.draw_dark_panel(surface, self.bg_rect)
        else:
