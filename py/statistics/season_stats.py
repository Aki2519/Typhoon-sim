...
def calculate_season_stats(
    ...
    for ty in sim.tys:
        ...
        # 检查是否在该洋区（以当年报点判断）
        if basin_area and not any(basin_area.contains(p['la'], p['lo']) for p in year_pts):
            continue

        # 系统整体数据（仅当年）
        max_wind = max(p['w'] for p in year_pts)
        ace = sum(p['pace'] for p in year_pts)
        name = sim.get_display_name(ty)

        # 风暴数统计（仅当年报点）
        has_td = any(p['w'] >= 29 and p['st'].upper() not in ('MD','SS','SD','EX','LO') for p in year_pts)
        has_ts = any(p['w'] >= 34 and p['st'].upper() not in ('MD','SS','SD','EX','LO') for p in year_pts)
        has_ty = any(p['w'] >= 64 and p['st'].upper() not in ('MD','SS','SD','EX','LO') for p in year_pts)
        has_mh = any(p['w'] >= 113 and p['st'].upper() not in ('MD','SS','SD','EX','LO') for p in year_pts)
        has_c5 = any(p['w'] >= 137 and p['st'].upper() not in ('MD','SS','SD','EX','LO') for p in year_pts)
        ...
        # 路径长度（仅当年 TS+ 报点间）
        ts_pts = [p for p in year_pts if _ace_eligible(p)]
        path_km = 0.0
        ...
        # TS+ 活跃时间段（去重叠）
        ts_active = [p for p in year_pts if _ace_eligible(p)]
        if ts_active:
            # 简化：取首尾时间差，实际应计算重叠
    ...
    # 登陆次数（从 sim.landfall_records 中统计）
    ...
    for lf in lf_records:
        if lf.get('year') == year and (basin_code is None or lf.get('basin') == basin_code):
            ...
            if stats['landfall_king'] is None or lf['wind'] > stats['landfall_king'][1]:
                stats['landfall_king'] = (lf.get('name', ''), lf['wind'])
