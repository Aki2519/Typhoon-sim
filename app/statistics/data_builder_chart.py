...
class ChartData:
    ...
    def __init__(self):
        ...
        self.typhoon_sort_data: List[Tuple[str, float, int, Tuple[int, int, int], str]] = []
        # (name, ace, peak_wind, color, start)
...
def build_chart_data(sim, year: int, cumulative_to_current: bool,
    ...
    # 台风 ACE 列表(缓存命中时避免急切求值默认参数)
    typhoon_cache = getattr(sim, '_ace_typhoon_cache', {})
    if year in typhoon_cache:
        data.typhoon_ace_list = typhoon_cache[year]
    else:
        data.typhoon_ace_list = engine.typhoon_ace_list(year)

    # 补充排序用信息（巅峰风速 + 颜色，法25: 复用台风级缓存）
    td = []
    name_to_ty = {}
    for ty in sim.tys:
    ...
    for name, ace in data.typhoon_ace_list:
        ...
        if ty is not None:
            cached = getattr(ty, '_cached_peak_info', None)
            if cached is None:
                from ..ty_sim_mixins._draw_icon_mixin import get_tropical_points
                tropical = get_tropical_points(ty.pts)
                if tropical:
                    mwp = max(tropical, key=lambda p: p['w'])
                    cached = (mwp['w'], tuple(mwp.get('color', (100, 150, 255))[:3]))
                else:
                    cached = (0, (100, 150, 255))
                ty._cached_peak_info = cached
            peak_wind, color = cached
