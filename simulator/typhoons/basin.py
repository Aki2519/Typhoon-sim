...
BASINS = {
    ...
    # MD: 地中海 0-360° 口径为西段(350-360,即 10°W-0°)
    # 与东段(0-36,即 0-36°E)。仅列西段(350-360)会漏捕东地中海系统(被 NI 截获),
    # 因此东段(0-36)与 NI 的 0-100E 重叠,须令 MD 先于 NI 判定。
    'MD': dict(name='地中海', check=lambda la, lo: 30 <= la <= 45 and (lo >= 350 or lo <= 36)),
...
def extratropical_pairs(tys: Optional[List[dict]] = None) -> List[Tuple[int, int, str]]:
    ...
    缓存仅作用于默认 miwu 全集: 显式传入 tys 时按实参重新计算,
    避免全局 _EX_CACHE 使显式样本集失效(混入其他月份/盆地样本)。"""
    global _EX_CACHE
    if tys is None and _EX_CACHE is not None:
        return _EX_CACHE
    src = tys if tys is not None else load_miwu()
    out = []
    for ty in src:
...
def _sample_bilinear(fld: np.ndarray, la: float, lo: float) -> float:
    """对 60°S..60°N / 0-360°E 1° 网格场做双线性采样(与 gen._bilinear_at 一致)。
    返回值: 实 NaN 时返回 np.nan(调用方须忽略);纬度越界时钳到边界行并线性外推
    (与 gen 口径一致;gpi 采样点均落在 60°S..60°N 网格内,实际不会触发)。"""
    h, w = fld.shape
    x = (lo % 360.0) / 360.0 * w
    y = (la + 60.0) / 120.0 * (h - 1)
    x0, y0 = int(x), int(y)
...
def genesis_gpi_threshold(gpi_map, tys=None) -> float:
    ...
    for ty in tys:
        ...
        if not pts:
            continue
        la, lo = pts[0][1], pts[0][2]
        # 用与 gen._bilinear_at 一致的双线性采样(文档声明"双线性"。
        # 旧实现 int(la+60)/int(lo%w) 为最近邻, 与 gen.py B5 及 docstring 不符。)
        v = _sample_bilinear(gpi, la, lo)
        if not np.isnan(v):
            vals.append(v)
