...
from .input_field import start_text_input_global, stop_text_input_global
from .script_engine import scan_scripts, Script

# IME 焦点持有者模型: script_dialog 内部同一时刻仅一个部件持有 IME,
# 切换/关闭时严格配对,避免计数泄漏导致中文输入残留(R2-7)。
# 注意: 只 start 不先 stop(底层 InputField 的计数保持,关闭本对话框后其 IME 不中断,T3-5)
_ime_owner = None


def _ime_acquire(owner, rect):
    global _ime_owner
    if _ime_owner is not owner:
        start_text_input_global(rect)
        _ime_owner = owner


def _ime_release(owner):
    global _ime_owner
    if _ime_owner is owner:
...
class _TextArea:
    ...
    def _snapshot(self):
        ...
        if self._undo_pos < 0 or self._undo_stack[self._undo_pos] != snap:
            self._undo_stack[self._undo_pos + 1:] = [snap]
            self._undo_pos += 1
    ...
    def _index_at(self, pos) -> tuple:
        ...
        while lo < hi:
            mid = (lo + hi) // 2
            if self.font.size(line[:mid + 1].replace('\t', '    '))[0] < rel_x:
                lo = mid + 1
            else:
                hi = mid
        if lo == 0:
            ...
            w_prev = self.font.size(line[:lo].replace('\t', '    '))[0]
            w_cur = self.font.size(line[:lo + 1].replace('\t', '    '))[0]
            col = lo - 1 if (rel_x - w_prev) < (w_cur - rel_x) else lo
    ...
    def _cut(self):
        if not self._scrap_ok:
            try:
                ...
                pass
        try:
            pygame.scrap.put(pygame.SCRAP_TEXT, self.lines[self.row].encode('utf-8'))
        except Exception:
            pass
    ...
    def _undo(self):
        ...
        if self._undo_pos > 0:
            self._undo_pos -= 1
        self.lines = list(self._undo_stack[self._undo_pos])
        if not self.lines:
            self.lines = ['']
        self.row = min(self.row, len(self.lines) - 1)
        self.col = min(self.col, len(self.lines[self.row]))
    ...
    def handle_event(self, e: pygame.event.Event) -> bool:
        ...
        # IME 组合结果：在光标处插入（R9 中文输入；N2: 非激活时不得插入正文）
        if e.type == pygame.TEXTINPUT and e.text:
            if not self.active:
                return False
            line = self.lines[self.row]
            self.lines[self.row] = line[:self.col] + e.text + line[self.col:]
            self.col += len(e.text)
            self._ensure_visible()
            self._notify()
            return True
        ...
        if e.key == pygame.K_RETURN:
            ...
            # 可打印字符由 TEXTINPUT 事件统一插入（避免与 KEYDOWN 双插）
            return True
        else:
            return False
...
class ScriptDialog(DraggableDialog):
    ...
    def deactivate(self):
        ...
        if self._editor is not None:
            ...
            _ime_release(self._editor)
        _ime_release(self)          # 文件名框等其他持有
        super().deactivate()
    ...
    def _open_editor(self, text: str = None, name: str = ""):
        ...
        if self._editor:
            ...
            # T3-6: 打开编辑器即启动 IME(否则首次点击文本区不会 acquire)
            _ime_acquire(self._editor, self._editor.rect)

    def _close_editor(self):
        if self._editor is not None:
            self._editor_text = self._editor.get_text()
            _ime_release(self._editor)
        _ime_release(self)           # N2: 退出编辑停用输入
        self.mode = 'list'
        self._refresh_scripts()
        self._update_rect()

    def _live_parse(self):
        """实时解析(防抖:仅置脏标志,由 draw 限频执行,法16)。"""
        self._parse_dirty = True

    def _flush_parse(self):
        if not getattr(self, '_parse_dirty', False):
            return
        now = pygame.time.get_ticks()
        if now - getattr(self, '_parse_last', -1000) < 200:
            return
        self._parse_last = now
    ...
    def _save_editor_script(self):
        ...
        if not name:
            name = "script.txt"
        # N12: 统一替换扩展名为 .json,并消毒文件名(禁止路径分隔符/穿越)
        name = os.path.splitext(os.path.basename(name))[0] + '.json'
    ...
    def _handle_list_event(self, e: pygame.event.Event) -> bool:
        ...
        if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
            ...
            # 点击脚本项（N11: 用 e.pos 实时命中测试,不用上一帧 _hovered_idx）
            list_x = r.x + 20
            list_y = r.y + 65
            list_w = r.width - 40
            hit_idx = -1
            for i in range(ITEMS_PER_PAGE):
                actual_idx = self._scroll_offset + i
                if actual_idx >= len(self.scripts):
                    break
                item_y = list_y + i * ITEM_H
                item_rect = pygame.Rect(list_x, item_y, list_w, ITEM_H)
    ...
    def _handle_editor_event(self, e: pygame.event.Event) -> bool:
        ...
        if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
            ...
            if name_rect and name_rect.collidepoint(e.pos):
                ...
                if self._editor:
                    self._editor.active = False
                    _ime_release(self._editor)
                # N2: 输入焦点切换给文件名框(中文可输入)
                _ime_acquire(self, name_rect)
                return True
            if self._editor and self._editor.rect.collidepoint(e.pos):
                # 激活交给 _TextArea.handle_event(下方 849 行分发),此处不重复计数
                self._name_editing = False
                _ime_release(self)

        # 文件名键入（含 IME 组合结果，N2）
        if getattr(self, '_name_editing', False) and e.type == pygame.TEXTINPUT and e.text:
            # 与 KEYDOWN unicode 插入去重(同一击键产生两个事件, K12)
            dedup = getattr(self, '_name_key_insert', None)
            if (dedup and dedup[0] == e.text and len(e.text) == 1
                    and pygame.time.get_ticks() - dedup[1] < 150):
                self._name_key_insert = None
                return True
            if len(self._editor_name) < 40:
                self._editor_name += e.text
            return True
        if getattr(self, '_name_editing', False) and e.type == pygame.KEYDOWN:
            ...
            if e.unicode and e.unicode.isprintable() and len(self._editor_name) < 40:
                ...
                self._name_key_insert = (e.unicode, pygame.time.get_ticks())
                return True

        if self._editor and self._editor.handle_event(e):
            return True

        # 编辑器模式禁止事件穿透到底层地图(含 TEXTINPUT/TEXTEDITING,
        # 否则 IME 组合事件会穿透到底层造成幽灵输入)
