...
def _parse_pt_time(t) -> Optional[datetime.datetime]:
    """解析报点时间字符串(10 位 YYYYMMDDHH 或 12 位 YYYYMMDDHHMM 含分钟)。
    失败返回 None。"""
    try:
        if len(t) >= 12:
            return datetime.datetime.strptime(t[:12], "%Y%m%d%H%M")
        return datetime.datetime.strptime(t[:10], "%Y%m%d%H")
...
class TyphoonDataMixin:
    ...
    def insert_point_by_time(self, pt) -> int:
        """按报点时间插入 pts，保持列表按时间升序（路径按时间连接）。
        时间无法解析时追加到末尾。返回插入后的索引。"""
        dt = _parse_pt_time(pt.get('t', ''))
        if dt is not None and self.pts:
            # pts 已按时间升序(本方法维护该不变式): 解析一次后二分定位
            times = [(_parse_pt_time(p.get('t', '')) or dt) for p in self.pts]
            lo = bisect.bisect_right(times, dt)
            self.pts.insert(lo, pt)
            return lo
        self.pts.append(pt)
        return len(self.pts) - 1

    def resort_point_by_time(self, idx: int) -> int:
        """报点时间被修改后，若顺序被破坏则移到按时间排序的正确位置。
        返回新索引；无需移动时返回原索引。"""
        n = len(self.pts)
        if n < 2 or idx < 0 or idx >= n:
            return idx
        dt = _parse_pt_time(self.pts[idx].get('t', ''))
        if dt is None:
            return idx
        prev_t = self.pts[idx - 1].get('t', '') if idx > 0 else ''
        next_t = self.pts[idx + 1].get('t', '') if idx + 1 < n else ''
        prev_ok = idx == 0 or (_parse_pt_time(prev_t) or dt) <= dt
        next_ok = idx == n - 1 or dt <= (_parse_pt_time(next_t) or dt)
        if prev_ok and next_ok:
            return idx
        pt = self.pts.pop(idx)
        return self.insert_point_by_time(pt)
    ...
    def add_point(self, t: str, la: float, lo: float, w: int, p: int,
        ...
        pace = 0.0
        st_up = st.upper()
        # ACE 公式: Σ(w²/10000), w 单位 kt, 仅热带性质且风速 ≥ 35kt 计
    ...
    def recalc_ace(self) -> None:
        ...
        geo_enabled = bool(self.sim and self.sim.ace_geo_limit_enabled
                           and self.sim.ace_engine is not None)
        for pt in self.pts:
            st = pt['st'].upper()
    ...
    def recalc_simulated_times(self) -> None:
        if not self.pts:
            ...
            self.points_dt = []
            return
        # C8: 起始时间跟随首报点(插入更早报点后季节定位/默认时间计算立即正确)
        self.start_time = self.pts[0]['t']

        self.points_dt = []
        for pt in self.pts:
            dt_parsed = _parse_pt_time(pt.get('t', ''))
            if dt_parsed is None:
                logger.warning(
                    f"台风 {getattr(self, 'n', '?')} 时间戳无效 '{pt.get('t', '')}'，"
                    f"回退为 2000-01-01，时间插值可能不准确")
                dt_parsed = datetime.datetime(2000, 1, 1, 0)
            self.points_dt.append(dt_parsed)
        ...
        off_map = {idx: k for k, idx in enumerate(officials)}
        for idx in range(len(self.pts)):
            li = bisect.bisect_right(officials, idx)
            left = officials[li - 1] if li > 0 else None
            ri = bisect.bisect_right(officials, idx)
            right = officials[ri] if ri < len(officials) else None
            self.points_time[idx] = self._interpolate_time(idx, left, right, off_map, off_times)
    ...
    def _interpolate_time(self, idx: int, left: Optional[int], right: Optional[int],
                          off_map: dict, off_times: List[float]) -> float:
        ...
        if left is not None and right is not None:
            left_t = off_times[off_map[left]]
            right_t = off_times[off_map[right]]
