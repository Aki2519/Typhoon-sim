...
功能栏最右侧「绘画」按钮可打开绘画模式窗口,用于把台风路径绘制成独立路径图(复刻 Typhoon4 / WikiProject Track Drawer 功能),可保存为 PNG 到 `picture/`。

- **文件选择**: 勾选当前已加载的台风(全选 / 全不选 / 滚轮滚动)。
- **范围**: `普通`(自动贴合路径范围) / `区域`(输入 `上,左,高,宽`,并带常用洋区预设) / `全局`(0–360° 全图)。
- **参数**: 色阶方案(应用现有 / 经典 Wiki / 2023 新版)、路径宽度、定点大小、半宽/半高留空、最小宽高、绘画顺序(同时/按强度排序/定位点置顶)。
- **显示选项**: 忽略前期扰动定位、显示所有定位点、显示经纬网、绘画风圈(不/全部/仅最后/仅热带)、多文件路径重置、文字标注、图例位置(不/左上/左下/右上/右下/自动)、背景色/地图底图、坐标单位(经纬度/像素)、自定义地图文件。
- **操作**: `开始绘画` 生成路径图预览, `保存路径图` 输出 PNG, `清除` 清空预览。
