...
def _parse_date(s: str) -> Optional[datetime]:
    ...
    分钟按小时取整（R24：时钟只显示小时，避免 ste 与显示差 59 分钟）。"""
    s = s.strip()
    if not s:
        return None
    ...
    if m:
        ...
        try:
            ...
            return None
        if mi:
            dt = dt.replace(minute=0, second=0) + timedelta(hours=1 if mi >= 30 else 0)
        return dt
...
class TargetRegion:
    ...
    dwell_speed: float = 1.0
    jump_on_depart: Optional[datetime] = None
    _has_departure_speed: bool = False
    _has_arrive_speed: bool = False
    _has_dwell_speed: bool = False
    _has_explicit_move_to: bool = False
    _has_explicit_move_from: bool = False
...
class _Parser:
    def __init__(self, text: str, filename: str, base_target_idx: int = 0):
        ...
        self._target_count = 0   # 法19: 运行计数,免每 target 全列表扫
    ...
    def parse(self) -> Script:
        while self._pos < len(self._lines):
            ...
            # 行尾内联注释: 去掉 ` # xxx`(README 示例在命令后带 # 注释)。
            # 任何合法命令都不含 '#',首个 '#' 即注释起点;仅注释则视为空行。
            if '#' in stripped:
            ...
            elif stripped.lower().startswith('hold '):
                # hold 5s → wait 5s
    ...
    def _parse_target_block(self, line: str):
        ...
        if width <= 0:
            raise ValueError(f"第{self._pos}行: 宽度必须大于 0: {m.group(3)}")

        idx = self._base_target_idx + self._target_count
    ...
    def _parse_sub_commands(self, tgt: TargetRegion):
        ...
        while self._pos < len(self._lines):
            ...
            if indent == 0 and stripped:
                break  # 无缩进非空行 → 回到顶层
            # 空行/纯空白行: 跳过, 不终止块(与 repeat 收集一致, 避免块内空行
            # 把后续缩进子命令挤成顶层而报"顶层命令不应有缩进"的边界错误)
            ...
            # 行尾内联注释(与顶层 dispatch 一致,见 parse())
            if '#' in stripped:
                stripped = stripped.split('#', 1)[0].rstrip()
                if not stripped:
                    continue
            ...
            if stripped.startswith('*'):
                ...
                if past_depart:
                    ...
                    tgt.dwell_speed = speed
                    tgt._has_dwell_speed = True
                else:
            ...
            if stripped.lower().startswith('arrive '):
                ...
                tgt.move_to_date = d
                tgt._has_explicit_move_to = True
                has_arrive = True
                past_depart = False
                continue
            ...
            if stripped.lower().startswith('depart '):
                ...
                tgt._has_explicit_move_from = True
                past_depart = True
                continue
    ...
    def _parse_repeat_block(self, line: str):
        ...
        # 法18: 块只解析一次成模板,后续迭代轻量克隆并重编号 target.index
        block_text = '\n'.join(block_lines)
        template = _Parser(block_text, self._filename, 0)
        template._pos = 0
        ...
        import copy
        base = self._target_count
        for k in range(count):
            iter_cmds = copy.deepcopy(template_script.commands)
            for c in iter_cmds:
                if c.type == CMD_TARGET:
                    c.target.index += base
            self._script.commands.extend(iter_cmds)
            base += n_per_iter
        # 展开后的目标也计入运行计数,避免其后顶层 target 复用/交错 index
        self._target_count += count * n_per_iter
...
class ScriptEngine:
    ...
    def start(self) -> bool:
        ...
        # R4-3: 统一走 _set_playing(清单点完成标志/记录手动切换)
        self.sim._set_playing(True)
    ...
    def _enter_target(self, tgt: TargetRegion):
        ...
        if self._prev_target is None:
            ...
            if tgt._has_dwell_speed:
                self._set_speed(tgt.dwell_speed)
            elif tgt._has_arrive_speed:
    ...
    def _update_dwell(self):
        ...
        if self._sim_abs_ste() >= end_abs:
            # 仅当 depart 是脚本显式给出的真实日期时才把模拟时间拨到该时刻;
            # 自动回退日期(2000-01-01)不得反向重写 sim 时间(否则无日期 target
            # 会被强制跳回 2000 年,R3 修复)。
            if tgt._has_explicit_move_from:
    ...
    def _start_moving_to(self, from_tgt: TargetRegion, to_tgt: TargetRegion):
        ...
        if self._interp_end_ste <= self._interp_start_ste:
            ...
            if to_tgt._has_dwell_speed:
                self._set_speed(to_tgt.dwell_speed)
            else:
                self._set_speed(to_tgt.approach_speed)
            self._state = self.STATE_DWELL
            return

        self._interp_inv_dur = 1.0 / (self._interp_end_ste - self._interp_start_ste)
    ...
    def _update_moving(self):
        ste_now = self._sim_abs_ste()
        if ste_now >= self._interp_end_ste:
            tgt = self._current_target
            # 结束虚拟拖动状态
            self.sim.right_button_dragging = False
            self.sim._drag_offset_x = 0
            ...
            for ty in self.sim.tys:
                self.sim._clear_drag_cache(ty)
            self._snap_to_target(tgt)
            # 与 _update_dwell 的 _has_explicit_move_from 守卫对称:
            # 仅当 arrive 是脚本显式真实日期时才把模拟时间拨到该时刻;
            # 自动回退日期(2000-01-01)不得反向重写 sim 时间,
            # 否则无 arrive 日期的 target 会经移动完成路径强制跳回 2000 年。
            if tgt._has_explicit_move_to:
        ...
        if self._interp_end_ste > self._interp_start_ste:
            t = (ste_now - self._interp_start_ste) * self._interp_inv_dur
        else:
            t = 1.0
        t = max(0.0, min(1.0, t))
    ...
    def _year_seconds_abs(year: int) -> float:
        total = 0.0
    ...
    for fname in sorted(os.listdir(script_dir)):
        ...
        try:
            with open(fpath, 'r', encoding='utf-8') as f:
                first_line = ""
                # 与 Script.parse 简介一致: 跳过首部空行/纯空白行,
                # 第一个 # 注释即简介(否则简介写在第二行时列表漏显)
                while True:
                    line = f.readline()
                    if not line:
                        break
        ...
        if first_line.strip().startswith('#'):
            description = first_line.strip()[1:].strip()
