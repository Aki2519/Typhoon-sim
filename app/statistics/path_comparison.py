...
class PathComparisonDialog(DraggableDialog):
    ...
    def _build(self):
        ...
        self._year = year
        # 按台风列表排序收集台风
        basin_order = build_basin_order(self.sim)
        ...
        self._tys = all_tys
        # 预计算每台风的年度过滤点集,供 _render 复用(勾选变化不重复过滤)
        self._ty_pts = {
            i: [p for p in ty.pts
                if p.get('ace_year', 0) == year and engine.point_in_limit(p['la'], p['lo'])]
        ...
        # 初始化布局（尽可能利用屏幕空间; 小窗口下限不超屏,R2-26）
        w = min(1800, max(800, self.sim.screen_width - 40))
        h = min(1100, max(560, self.sim.screen_height - 60))
    ...
    def _render(self):
        ...
        # ── Phase 1: 计算地理范围(复用 _build 预计算的点集) ──
        all_lons, all_lats = [], []
        ty_pts_cache = {}
        for idx, ty in enumerate(self._tys):
            if idx not in self._selected:
                continue
            pts = self._ty_pts.get(idx)
            if not pts:
                continue
            ty_pts_cache[idx] = pts
            for p in pts:
        ...
        # R2-01: 经度归一到 0-360 制,与 render_map_subset 的环绕切片一致
        mlon = mlon % 360.0
        Mlon = Mlon % 360.0
        if Mlon <= mlon:
            Mlon += 360.0
        ...
        # ── Phase 3: 坐标映射与路径绘制 ──
        inv_lon = box_w / (Mlon - mlon)
        inv_lat = box_h / (Mlat - mlat)

        def geo_to_local(lon, lat):
            # R2-01: 跨 0 度线窗口内数据点归一到窗口坐标系(取最近等价位置)
            d = (lon - mlon) % 360.0
            if d > (Mlon - mlon):
                d -= 360.0
            x = box_x + d * inv_lon
            y = box_y + (Mlat - lat) * inv_lat
            return int(x), int(y)
        ...
        for idx, ty in enumerate(self._tys):
            ...
            if screen_pts:
                ...
                # 标签挂在路径末端,避免首点落在窗口外时名称不可见
                surf.blit(ns, (screen_pts[-1][0] + 5, screen_pts[-1][1] - 8))
            ...
            if box_rect.colliderect(cb_rect):
                if ctrl:
                    if idx in self._selected:
                        ...
                        self._selected.add(idx)
                        changed = True
                elif idx not in self._selected:
                    self._selected.add(idx)
                    changed = True
